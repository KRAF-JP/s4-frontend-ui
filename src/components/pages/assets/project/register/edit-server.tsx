...
type Props = {
  ...
  isLoading: boolean
  setVersionDefaultData: any
  versionDefaultData: any
  setOsDefaultData: any
  osDefaultData: any
  className?: string
}

const EditServer: React.FC<Props> = (props) => {
  ...
  const handleOsFamily = (formValue: any) => {
    ...
    if (server_os_family === 'windows') {
      ...
      props.setOsDefaultData(props.windowsOsList[0])
    ...
    if (server_os_family !== 'windows') return
    props.setData({
      ...props.data,
  ...
  useEffect(() => {
    if (props.data.os_family === 'windows') {
      setIsOther(false)
      setIsWindows(true)
      setOs(props.data.os_family)
      props.setWindowsOs(props.data.os_name)
      props.setVersionDefaultData({
        label: props.data.os_version,
        value: props.data.os_version,
      })
      props.setOsDefaultData({
        label: props.data.os_name.replace('_', ' '),
        value: props.data.os_name,
      })
    } else if (props.data.os_family === 'other') {
      setIsOther(true)
      setIsWindows(false)
      setOs(props.data.os_family)
    } else {
      ...
      setOs(props.data.os_family)
      props.setVersionDefaultData({
        label: props.data.os_release,
        value: props.data.os_release,
      })
    }
  }, [props.data.os_family])
  ...
  return (
    <>
      <Form
        ...
        initialValues={{
          server_name: props.data.name,
          server_os_family: props.data.os_family,
          update_means: props.data.auto_update_enabled,
          installDate: props.data.last_modified_date
            ? props.data.last_modified_date.replaceAll('-', '/')
            : null,
        }}
        validate={(values: any) => {
          const errors: any = {}
            ...
            <FieldFlex>
              ...
              {isWindows && (
                <FormField label={'OS'} marginBottom={24} required>
                  <Select
                    defaultData={props.osDefaultData}
              ...
              {!isOther && (
                <FormField label={'バージョン'} marginBottom={24} required>
                  <Select
                    defaultData={props.versionDefaultData}
                  ...
                  <Field name={'update_means'} type={'radio'} value={'true'}>
                    {({ input, meta }) => (
                      <ToggleTab
                        {...input}
                        label={'自動更新'}
                        checked={props.data.auto_update_enabled}
                        onChange={(e) => {
                          input.onChange(e)
